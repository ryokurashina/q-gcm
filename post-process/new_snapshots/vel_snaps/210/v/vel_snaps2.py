    ...
    for j in range(3):
        if i == 0:
            ...
            if variable == 0:
                ...
                s.take_snapshots(1, 365, savepath, True)
        else:
            dx = dxa
            # Dimensionalise alpha
            alpha = bccoat/dx
            p_at = utils.Pressure(atpa.variables['p'][steps, j, :, :])
            if variable == 0:
                name = 'u_at'+'_'+str(j+1)+'_a'
                u = utils.ContourData(p_at.zonal(1, alpha, dx, fnot).data, name=name)
                [fig, ax] = u.init_frame('Zonal Velocity ($ms^{-1}$)', False)
                u.take_snapshots(1, 365, savepath, False)
            elif variable == 1:
