...
datapath = '/rds/general/user/rk2014/home/WORK/q-gcm/outdata_toptest/test/1_coupled/'
...
for i in range(2):
    for j in range(3):
        ...
        if i == 0:
            ...
            anim.save('./pv'+str(j+1)+'_oc_a.gif', writer=writer)
        else:
            [fig, ax] = q.init_frame('Potential Vorticity ($s^{-1}$)', False)
            anim = ani.FuncAnimation(fig, q.frame_i, frames=time)
            anim.save('./pv'+str(j+1)+'_at_a.gif', writer=writer)
